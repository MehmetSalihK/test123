...
exports.run = (client, message, args) => {
  ...
  if (!modlog) return message.reply('`ban-bilgi` kanalını bulamıyorum.');
  if (reason.length < 1) return message.reply('Kick sebebini yazmalısın.');
  if (message.mentions.users.size < 1) return message.reply('Kimi Kickleyeceğimi yazsana Oğlim. :)').catch(console.error);
